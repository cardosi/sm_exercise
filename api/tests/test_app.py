...
def test_put_pizza(mock_update_item, client):
    test_pizza = {
        "id": 1,
        "name": "Updated Pizza",
        "created_at": "2021-01-01T00:00:00Z",
    ...
    response = client.put('/pizzas/1', json=test_pizza)
    data = json.loads(response.data)
    assert response.status_code == 200
    assert data['id'] == 1
...
def test_delete_pizza(mock_delete_item, client):
    pizza_topping_ids = [1, 2, 3]
    mock_delete_item.return_value = None
    response = client.delete(
        '/pizzas/1', json={"pizza_topping_ids": pizza_topping_ids})
...
def test_put_topping_not_found(mock_get_item, mock_update_item, client):
    mock_update_item.return_value = None
    test_topping = {
        "name": "Sample Topping 3"
    }
    response = client.put('/toppings/2', json=test_topping)
    data = json.loads(response.data)
...
@patch('database.DynamoDBClient.update_item')
@patch('database.DynamoDBClient.get_item')
def test_put_topping(mock_get_item, mock_update_item, client):
    test_topping = {
        "id": 2,
        "name": "Sample Topping 2"
    }
    ...
    response = client.put('/toppings/2', json={"name": "Updated Topping"})
    data = json.loads(response.data)
    assert response.status_code == 200
    assert data['id'] == 2
...
def test_delete_topping(mock_delete_item, client):
    mock_delete_item.return_value = None
    response = client.delete(
        '/toppings/1', json={"pizza_topping_ids": [1, 2, 3]})
    assert response.status_code == 204
